...
export default class Quaternion {
  ...
  static fromVectors(from, to) {
    ...
    if (d >= 1) {
      ...
      let axis = Vector3.X_AXIS.cross(from);
      axis = axis.length !== 0 ? axis : Vector3.Y_AXIS.cross(from);
      axis = axis.normalize();
      return Quaternion.fromAxisAngle(axis, Math.PI);
    } else {
      const s = Math.sqrt((1 + d) * 2);
      const { x, y, z } = from.cross(to);

      return new Quaternion(s / 2, x / s, y / s, z / s).normalize();
    }
  }
  ...
  toAngleAround(axis) {
    ...
    const signed = orthonormal
      .clone()
      .cross(flattened)
      .normalize();

    return -Math.sign(axis.dot(signed)) * Math.acos(orthonormal.dot(flattened));
  }
  ...
  toEulerAngles() {
    ...
    if (test > 0.499) {
      ...
      let yaw = 2 * Math.atan2(this.x, this.w);
      let pitch = Math.PI / 2;
      let roll = 0;
