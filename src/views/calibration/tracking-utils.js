...
export const usesFloorOffsetModel = (tracker) => tracker && tracker.setFloorOffset

export const usesSimpleGroundConfirm = (tracker) => tracker && tracker.confirmGroundPosition

export const initTracking = (tracker) => tracker && tracker.reset()

export const activateARCamera = ({cameras}) => cameras.arCamera && cameras.arCamera.activate()

export const getDeviceType = ({appConfig}) => appConfig.deviceType

export const insertFloorOffsetModel = async({modelManager, sceneManager}) => {
  const model = modelManager.findModelByForeignKey(FLOOR_MODEL_FOREIGN_KEY) || await modelManager.getModelFromRepository(FLOOR_MODEL_FALLBACK_ID)
  if (model) {
    await sceneManager.insertModel(model, {id: FLOOR_MODEL_ID, pose: DEFAULT_POSE})
  } else {
...
export const scaleFloorOffsetModel = async(tracker, factor) => tracker.setFloorOffset(tracker.floorOffset + factor)
